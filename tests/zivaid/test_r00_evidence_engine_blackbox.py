...
def valid_record():
    return {
        "evidence_id": "BBX-R00-001", "participant_id": "BBX-P-001", "wave_id": "R00-W01",
        "protocol_version": "R00-v0.2", "instrument_id": "R00-INSTR-v0.2", "concept_id": "",
        "researcher_id": "BBX-R-001", "channel": "street", "segment_primary": "migrant",
...
class TestR00EvidenceEngineBlackBox(unittest.TestCase):
    ...
    def test_001_valid_record_is_usable(self):
        ...
        self.assertTrue(result["counts_for_hypotheses"]); self.assertFalse(result["review_required"])

    def test_002_missing_required_identifier_is_not_usable(self):
        record = valid_record(); del record["evidence_id"]
        result = self.assert_decision(record, "QUARANTINE"); self.assertFalse(result["counts_for_hypotheses"])

    def test_003_invalid_instrument_is_not_usable(self):
        record = valid_record(); record["instrument_id"] = "R00-INSTR-v0.1"
        self.assert_decision(record, "QUARANTINE")

    def test_004_unknown_sequence_is_quarantined(self):
        record = valid_record(); record["discovery_before_concept"] = "unknown"
        self.assert_decision(record, "QUARANTINE")

    def test_005_prohibited_sensitive_field_is_excluded(self):
        record = valid_record(); record["passport_number"] = "SYNTHETIC-NOT-REAL"
        result = self.assert_decision(record, "EXCLUDED"); self.assertFalse(result["counts_for_hypotheses"])
    ...
    def test_007_contradiction_is_not_invalidity(self):
        record = valid_record(); record["contradiction"] = "yes"
    ...
    def test_008_unknown_controlled_value_is_quarantined(self):
        record = valid_record(); record["frequency"] = "unknown-value"
        self.assert_decision(record, "QUARANTINE")
    ...
    def test_010_validation_does_not_mutate_or_create_production_records(self):
        record = valid_record(); before = copy.deepcopy(record)
        result = validate(record, now="2026-08-17T00:00:00+00:00")
